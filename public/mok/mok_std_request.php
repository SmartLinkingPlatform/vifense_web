...
<?php
    ...
    $key_path = __DIR__."/mok_dev_keyInfo.dat"; // dev
    $key_path = __DIR__."/mok_dev_keyInfo.dat"; // product
    $password = "Dgtmoby123!@#";
    $mobileOK->key_init($key_path, $password);

    // 이용기관 거래ID생성시 이용기관별 유일성 보장을 위해 설정, 이용기관식별자는 이용기관코드 영문자로 반드시 수정
    $clientPrefix = "VIFENSE";     // 8자이내 권고 (예) MOK, TESTCOKR
...
<?php
    ...
    function mobileOK_std_request($mobileOK, $result_return_url, $clientPrefix) {
        ...
        $req_client_info = $client_tx_id."|".$date_time; // default
        //$req_client_info = $date_time."|".$client_tx_id; // add ckd
